...
def euler_lagrange_residuals(model, t):
    """Compute the two Euler-Lagrange residuals directly with autograd.

        f1 = d/dt(dL/dtheta1_dot) - dL/dtheta1
        f2 = d/dt(dL/dtheta2_dot) - dL/dtheta2
    """
    theta = model(t)
    ...
    _, _, lagrangian, energy = mechanics(theta1, theta2, omega1, omega2)

    dL_domega1 = torch.autograd.grad(
        lagrangian,
        omega1,
    ...
    dL_domega2 = torch.autograd.grad(
        lagrangian,
        omega2,
        grad_outputs=torch.ones_like(lagrangian),
        create_graph=True,
    ...
    dL_dtheta1 = torch.autograd.grad(
        lagrangian,
        theta1,
        grad_outputs=torch.ones_like(lagrangian),
        create_graph=True,
    ...
    dL_dtheta2 = torch.autograd.grad(
        lagrangian,
        theta2,
        grad_outputs=torch.ones_like(lagrangian),
        create_graph=True,
    ...
    dt_dL_domega1 = derivative(dL_domega1, t)
    dt_dL_domega2 = derivative(dL_domega2, t)

    f1 = dt_dL_domega1 - dL_dtheta1
    f2 = dt_dL_domega2 - dL_dtheta2
...
def main():
    ...
    for epoch in range(EPOCHS + 1):
        ...
        theta_physics, omega1_physics, omega2_physics, energy_physics, f1, f2 = (
            euler_lagrange_residuals(model, t_physics)
        )

        # 2) Physics loss: both Euler-Lagrange equations.
