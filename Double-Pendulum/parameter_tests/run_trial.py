...
out=ROOT/'parameter_tests'/name; out.mkdir(exist_ok=True)
...
if kind=='fourier':
 for node in ast.walk(tree):
  if isinstance(node,ast.keyword) and node.arg=='milestones': node.value=ast.parse('[10000,30000,60000]',mode='eval').body
if kind=='time':
 for node in ast.walk(tree):
...
def observe(v):
 ...
 full=r2(ref,pred); extra=r2(ref[300:],pred[300:]); good=min(full+extra)>=.999 and physics<=1e-5
...
try:
 ...
 (out/'result.json').write_text(json.dumps(dict(name=name,model=kind,params=params,scheduler_milestones=[10000,30000,60000] if kind=='fourier' else None,source_sha256=hashlib.sha256(raw.encode()).hexdigest(),status=status,history=rows,elapsed_seconds=time.perf_counter()-start),indent=2))
